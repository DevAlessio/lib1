local SettingsButton = {
    Rectangle = { Y = 0, Width = 500, Height = 48 },
    Text = { X = 13, Y = 18, Scale = 0.25 },
    LeftBadge = { Y = -2, Width = 40, Height = 40 },
    RightBadge = { X = 385, Y = -2, Width = 40, Height = 40 },
    RightText = { X = 420, Y = 4, Scale = 0.25 },
    SelectedSprite = { Dictionary = "commonmenu", Texture = "gradient_nav", Y = 10, Width = 440, Height = 45 },
}

---@type table
local SettingsList = {
    LeftArrow = { Dictionary = "commonmenu", Texture = "arrowleft", X = 378, Y = 3, Width = 28, Height = 45 },
    RightArrow = { Dictionary = "commonmenu", Texture = "arrowright", X = 400, Y = 3, Width = 28, Height = 45 },
    Text = { X = 395, Y = 18, Scale = 0.25 },
}
    ...
    if CurrentMenu ~= nil then
        if CurrentMenu() then
            ...
            if CurrentMenu.Pagination.Minimum <= Option and CurrentMenu.Pagination.Maximum >= Option then
                ...
                if not Active then
                    RenderRectangle(CurrentMenu.X + 15, CurrentMenu.Y + 20 + SettingsButton.SelectedSprite.Y + CurrentMenu.SubtitleHeight + RageUI.ItemOffset, SettingsButton.SelectedSprite.Width + CurrentMenu.WidthOffset - 55, SettingsButton.SelectedSprite.Height - 1, 16, 16, 16, 90)
                end
    
                if Selected then 
                    RenderRectangle(CurrentMenu.X + 15, CurrentMenu.Y + 20 + SettingsButton.SelectedSprite.Y + CurrentMenu.SubtitleHeight + RageUI.ItemOffset, SettingsButton.SelectedSprite.Width + CurrentMenu.WidthOffset - 55, SettingsButton.SelectedSprite.Height - 1, 96, 36, 36, 255)
                end
                ...
                if Enabled == true or Enabled == nil then
                    if Selected then
                        if Style.RightLabel ~= nil and Style.RightLabel ~= "" then
                            RenderText(Style.RightLabel, CurrentMenu.X + SettingsButton.RightText.X - RightBadgeOffset + CurrentMenu.WidthOffset, CurrentMenu.Y + SettingsButton.RightText.Y + CurrentMenu.SubtitleHeight + RageUI.ItemOffset, 13, SettingsButton.RightText.Scale, 0, 0, 0, 255, 2)
                ...
                if Enabled == true or Enabled == nil then
                    if Selected then
                        RenderText(Label, 10 + CurrentMenu.X + SettingsButton.Text.X + LeftBadgeOffset, 21 + CurrentMenu.Y + SettingsButton.Text.Y + CurrentMenu.SubtitleHeight + RageUI.ItemOffset, 13, SettingsButton.Text.Scale, 255, 255, 255, 255)
                        RenderText(ListText, CurrentMenu.X + SettingsList.Text.X + CurrentMenu.WidthOffset - RightOffset, 21 + CurrentMenu.Y + SettingsList.Text.Y + CurrentMenu.SubtitleHeight + RageUI.ItemOffset, 13, SettingsList.Text.Scale, 255, 255, 255, 255, 2)
                    else
                        RenderText(Label, 10 + CurrentMenu.X + SettingsButton.Text.X + LeftBadgeOffset, 21 + CurrentMenu.Y + SettingsButton.Text.Y + CurrentMenu.SubtitleHeight + RageUI.ItemOffset, 13, SettingsButton.Text.Scale, 255, 255, 255, 255)
                        RenderText(ListText, CurrentMenu.X + SettingsList.Text.X + CurrentMenu.WidthOffset - RightOffset, 21 + CurrentMenu.Y + SettingsList.Text.Y + CurrentMenu.SubtitleHeight + RageUI.ItemOffset, 13, SettingsList.Text.Scale, 255, 255, 255, 255, 2)
                    end
                else
                    RenderText(Label, 28 + CurrentMenu.X + SettingsButton.Text.X + LeftBadgeOffset, 21 + CurrentMenu.Y + SettingsButton.Text.Y + CurrentMenu.SubtitleHeight + RageUI.ItemOffset, 13, SettingsButton.Text.Scale, 255, 255, 255, 255)
                    
                    if Selected then
                        RenderText(ListText, 28 + CurrentMenu.X + SettingsList.Text.X + CurrentMenu.WidthOffset, 21 + CurrentMenu.Y + SettingsList.Text.Y + CurrentMenu.SubtitleHeight + RageUI.ItemOffset, 13, SettingsList.Text.Scale, 255, 255, 255, 255, 2)
                    else
                        RenderText(ListText, 28 + CurrentMenu.X + SettingsList.Text.X + 5 + CurrentMenu.WidthOffset, 21 + CurrentMenu.Y + SettingsList.Text.Y + CurrentMenu.SubtitleHeight + RageUI.ItemOffset, 13, SettingsList.Text.Scale, 255, 255, 255, 255, 2)
                    end
                end
