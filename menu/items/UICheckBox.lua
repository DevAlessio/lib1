...
local SettingsButton = {
    ...
    Text = { X = 17, Y = 16, Scale = 0.25 },
    LeftBadge = { Y = -2, Width = 40, Height = 40 },
    RightBadge = { X = 385, Y = -2, Width = 40, Height = 40 },
    RightText = { X = 420, Y = 3, Scale = 0.30 },
    SelectedSprite = { Dictionary = "commonmenu", Texture = "gradient_nav", Y = 8, Width = 440, Height = 45 },
}
    ...
    if CurrentMenu ~= nil then
        if CurrentMenu() then
            ...
            if CurrentMenu.Pagination.Minimum <= Option and CurrentMenu.Pagination.Maximum >= Option then
                ...
                if Selected then 
                    RenderRectangle(CurrentMenu.X + 15, CurrentMenu.Y + 20 + SettingsButton.SelectedSprite.Y + CurrentMenu.SubtitleHeight + RageUI.ItemOffset, SettingsButton.SelectedSprite.Width + CurrentMenu.WidthOffset - 55, SettingsButton.SelectedSprite.Height - 1, 96, 36, 36, 215)
                end

                if type(Style) == "table" then
                    if Style.Enabled == true or Style.Enabled == nil then
                        if Selected then
                            RenderText(Label, 10 + CurrentMenu.X + SettingsButton.Text.X + LeftBadgeOffset, 21 + CurrentMenu.Y + SettingsButton.Text.Y + CurrentMenu.SubtitleHeight + RageUI.ItemOffset, 13, SettingsButton.Text.Scale, 245, 245, 245, 255)
                        else
                        ...
                        if Selected then
                            RenderText(Label, 10 +  CurrentMenu.X + SettingsButton.Text.X + LeftBadgeOffset, 21 + CurrentMenu.Y + SettingsButton.Text.Y + CurrentMenu.SubtitleHeight + RageUI.ItemOffset, 13, SettingsButton.Text.Scale, 0, 0, 0, 255)
                        else
                            RenderText(Label, 10 + CurrentMenu.X + SettingsButton.Text.X + LeftBadgeOffset, 21 + CurrentMenu.Y + SettingsButton.Text.Y + CurrentMenu.SubtitleHeight + RageUI.ItemOffset, 13, SettingsButton.Text.Scale, 255, 255, 255, 255)
                        end
                    ...
                    if Style.Enabled == true or Style.Enabled == nil then
                        if Selected then
                            if Style.RightLabel ~= nil and Style.RightLabel ~= "" then
                                ...
                                RenderText(Style.RightLabel, 10 + CurrentMenu.X + SettingsButton.RightText.X - RightBadgeOffset + CurrentMenu.WidthOffset, 21 + CurrentMenu.Y + SettingsButton.RightText.Y + CurrentMenu.SubtitleHeight + RageUI.ItemOffset, 13, SettingsButton.RightText.Scale, 0, 0, 0, 255, 2)
                                BoxOffset = MeasureStringWidth(Style.RightLabel, 0, 0.35)
                            end
                        else
                            if Style.RightLabel ~= nil and Style.RightLabel ~= "" then
                                RenderText(Style.RightLabel, 10 + CurrentMenu.X + SettingsButton.RightText.X - RightBadgeOffset + CurrentMenu.WidthOffset, 21 + CurrentMenu.Y + SettingsButton.RightText.Y + CurrentMenu.SubtitleHeight + RageUI.ItemOffset, 13, SettingsButton.RightText.Scale, 245, 245, 245, 255, 2)
                                BoxOffset = MeasureStringWidth(Style.RightLabel, 0, 0.35)
                            end
                        end
                    end
